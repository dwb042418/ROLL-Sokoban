...
class MegatronInferStrategy(InferenceStrategy):
    ...
    def __init__(self, worker: Worker):
        ...
        # Manually merge additional_configs since get_config_dict() may not be available
        config_dict = self.worker_config.training_args.to_dict()
        # Merge additional_configs if it exists
        additional_configs = getattr(self.worker_config.training_args, 'additional_configs', None)
        if additional_configs:
            if isinstance(additional_configs, str):
                import json
                with open(additional_configs, 'r') as f:
                    additional_configs = json.load(f)
    ...
    def inner_forward_step(self, loss_func, data_iterator: Iterator[DataProto], model):
        ...
        # For causal models with Megatron Core, we don't pass 2D attention_mask
        # to avoid shape mismatch. The model will handle causal masking internally.
        attention_mask = None

        if self.use_remove_padding and attention_mask is not None:
        ...
        if self.use_sequence_packing:
            # For sequence packing, attention_mask is required
            if attention_mask is None:
                raise ValueError("attention_mask is required for sequence packing but was not provided")
...
class MegatronTrainStrategy(MegatronInferStrategy, TrainStrategy):
    ...
    def initialize(self, model_provider):
        ...
        optimizer_config = OptimizerConfig(
            ...
            min_lr=self.megatron_train_args.lr_scheduler_kwargs.get("min_lr", 0.0) if self.megatron_train_args.lr_scheduler_kwargs else 0.0,
        ...
        # Ensure warmup_steps is at most max_steps - 1 to avoid assertion error
        warmup_steps = self.megatron_train_args.get_warmup_steps(self.megatron_train_args.max_steps)
        if warmup_steps >= self.megatron_train_args.max_steps:
            logger.warning(
                f"warmup_steps ({warmup_steps}) >= max_steps ({self.megatron_train_args.max_steps}), "
                f"setting warmup_steps to 0 and warmup_ratio to 0"
            )
            self.megatron_train_args.warmup_steps = 0
            self.megatron_train_args.warmup_ratio = 0

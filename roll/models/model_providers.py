...
def default_tokenizer_provider(model_args: "ModelArguments", model_name_or_path: str=None):
    ...
    if tokenizer.pad_token is None:
        if tokenizer.eos_token is not None:
            ...
            logger.info("Added special pad_token: [PAD]")

    # Double-check that pad_token is set
    if tokenizer.pad_token is None:
        logger.warning(f"Tokenizer still has no pad_token after attempting to set it. Model: {model_name_or_path}")
        # Force set it as a last resort
        if tokenizer.eos_token is not None:
            tokenizer.pad_token = tokenizer.eos_token
            tokenizer.pad_token_id = tokenizer.eos_token_id
            logger.warning(f"Force set pad_token to eos_token (id={tokenizer.eos_token_id})")
